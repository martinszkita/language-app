...
def add_word(request, language_str):
    ...
    return render(request, 'vocabulary/add_word.html', {'language': language_str, 'form': form})
    # if request.method == 'POST':
    #     form = WordForm(request.POST)
    #     if form.is_valid():
    #         word = form.save(commit=False)
    #         lang = resolve(request.path_info).kwargs.get('language')
    #         word.language = Language.objects.get(name=lang) 
